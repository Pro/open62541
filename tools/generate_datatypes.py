...
def parseTypeDefinitions(outname, xmlDescription, namespace, addToTypes=None):
    ...
    while(len(snippets) > 0):
        if detectLoop == len(snippets):
            ...
            raise RuntimeError("Infinite loop detected or type not found while processing types " + name + ": unknonwn subtype " + str(unknownTypes(typeXml)) +
                               " Maybe you need to import additional types with the --import flag. E.g. '--import==UA_TYPES#/path/to/deps/ua-nodeset/Schema/Opc.Ua.Types.bsd'")
